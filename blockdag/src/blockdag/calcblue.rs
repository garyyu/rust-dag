...
pub fn calc_blue(block_name: &str, node: &mut Node, k: i32){
    ...
    let mut block = block.unwrap().write().unwrap();

    if block.name=="Genesis" {
        block.is_blue = true;
        return;
    }

    // step 2
    let mut tip_max_name = block.name.clone();
    let mut max_past_blue: u64 = block.size_of_past_blue;
    let tips = &node.tips;
    if tips.len() == 0 {
    ...
    // step 3
    if &tip_max_name == &block.name {

        // step 4
        block.is_blue = true;

        // step 5
        for (name, value) in &block.prev {
            ...
            // step 6
            let (blues,blue_anticone) = tips_anticone_blue(name, &block.prev, k);
            if blues>=0 && blues<=k {
                // step 7
        ...
        // step 11
        let (blues,blue_anticone) = tips_anticone_blue(&block.name, tips, k);
        if blues>=0 && blues<=k {

            // step 12
            //let pred = &Arc::clone(value).write().unwrap();
            block.is_blue = true;
            block.size_of_anticone_blue = blues;
